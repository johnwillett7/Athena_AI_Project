...
from athena_all.sem_parser.grammar._scoring import Model, score
...
def latent_sgd(
    ...
    """print("=" * 80)
    print(
        "Running SGD learning on %d examples with training metric: %s\n"
    ...
    for t in range(T):
        ...
        for example in examples:
            ...
            if good_parses:
                ...
                scores = [(p.score + _cost(target_parse, p), p) for p in parses]
                ...
                predicted_parse = random.choice(
                    [p for s, p in scores if s == max_score]
                )
                if training_metric.evaluate(example, parses):
                    num_correct += 1
                ada_update_mag, adagrad = _update_weights(
                    model,
                    target_parse,
                    predicted_parse,
                    eta,
        ...
        if error < epsilon or ada_update_mag < epsilon:
            break
    _print_weights(model.weights)
    return model


def _cost(parse_1, parse_2):
...
def _update_weights(
    model, target_parse, predicted_parse, eta, l2_penalty, adagrad, ada_update_mag
):
...
def _print_weights(weights, n=20):
...
def demo_learning_from_semantics(domain):
    ...
    model = domain.model()
    model = latent_sgd(
        model=model, examples=examples, training_metric=SemanticsAccuracyMetric()
    )
...
def demo_learning_from_denotations(domain):
    ...
    model = domain.model()
    model = latent_sgd(
        model=model, examples=examples, training_metric=DenotationAccuracyMetric()
    )
