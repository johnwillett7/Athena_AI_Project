...
# TODO: comment
def print_sample_outcomes(
    model=None, examples=[], name=None, metric=None, metric_test=None, k=10
):
    candidates = []
...
# TODO: comment
def sample_wins_and_losses(domain=None, model=None, metric=None, seed=None):
    if seed:
        random.seed(seed)
    ...
    def my_print_sample_outcomes(name=None, metric_test=None, k=10):
        examples = print_sample_outcomes(
            model=model,
            examples=train_examples,
            name=name,
            metric=metric,
...
# TODO: comment
def print_parses(
    example, parses, metrics=[NumParsesMetric()], max_parses=10000, show_syntax=False
):
    ...
    for idx, parse in enumerate(parses[:max_parses]):
        ...
        if show_syntax:
            lines.append(
                "%-15s %s   \n%s"
                % ("parse", parse_outcome, parse.parse_to_pretty_string())
            )
        lines.append(
            "%-15s %s   %s" % ("semantics", semantics_outcome, parse.semantics)
        )
        if example.denotation != None or parse.denotation != None:
            lines.append(
                "%-15s %s   %s" % ("denotation", denotation_outcome, parse.denotation)
            )
...
def evaluate_model(
    ...
    print_examples=True,
    print_all=False,
    print_test=lambda metric: metric == 0,
    test_metric=SemanticsOracleAccuracyMetric(),
):
    print("=" * 80)
    print(
        "Evaluating on %d %sexamples\n"
    ...
    for example in examples:
        ...
        if print_examples and (
            print_test(test_metric.evaluate(example, parses)) or print_all
        ):
    ...
    for metric in metrics:
        print(
            "%-34s %.3f"
            % (metric.name(), 1.0 * metric_values[metric.name()] / len(examples))
        )
    print()
...
def evaluate_grammar(
    ...
    evaluate_model(
        model=Model(grammar=grammar, executor=executor),
        examples=examples,
        metrics=metrics,
...
def test_executor(domain):
    ...
    for example in domain.examples():
        ...
        print(
            "%-24s %s"
            % ("actual denotation", "[match]" if match else actual_denotation)
        )
        print()


def evaluate_for_domain(domain, print_examples=True, print_all=True):
    print("#" * 80)
    ...
    which_examples = (
        "ONLY PRINTING UN-PARSED EXAMPLES."
        if not print_all
        else "PRINTING ALL EXAMPLESS."
    )
    print("#" * 80)
    print(which_examples)
    print("#" * 80)
...
def train_test(
    ...
    # model.grammar.print_grammar()
    # print
    print(
        "%d training examples, %d test examples"
    ...
    # Train
    model = latent_sgd(
        model, train_examples, training_metric=training_metric, seed=seed
    )
...
def learn_lexical_semantics(domain, seed=None):
    ...
    print("#" * 80)
    print(
        "Learn lexical semantics experiment for domain: %s\n"
        % domain.__class__.__name__
    )
